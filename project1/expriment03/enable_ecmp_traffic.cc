...
int main (int argc, char *argv[]) {
    ...
    unsigned int nSer = 8;
    unsigned int nToR = 4;
    unsigned int nAggr = 4;
    unsigned int nCore = 2;
  
    // get nodes
    NodeContainer nodes[4];
    nodes[0].Create (nSer); // 8 servers
    nodes[1].Create (nToR); // 4 ToR switches
    nodes[2].Create (nAggr); // 4 Aggregation switches
    nodes[3].Create (nCore); // 2 core switch
    unsigned int totalSeg = nToR + nAggr + nAggr;

    NodeContainer networkSegment[totalSeg];
    for (unsigned int i = 0; i < totalSeg; ++i) {
        if (i < nToR) {
            ...
            networkSegment[i].Add (nodes[2].Get(i-nAggr));
            networkSegment[i].Add (nodes[1].Get(2*((i-nAggr)/2)));
            networkSegment[i].Add (nodes[1].Get(2*((i-nAggr)/2)+1));
        }
        else {
            if ((i-(nToR + nAggr))%2 == 0)
                networkSegment[i].Add (nodes[3].Get(0));
            else
                networkSegment[i].Add (nodes[3].Get(1));
            networkSegment[i].Add (nodes[2].Get(i-(nToR+nAggr)));
        }
    }
    ...
    //     tcpConnect(nodes[0].Get(0), interfacesToR[0].GetAddress(1), port+11+i, nodes[0].Get(i), 1, 60, 1, 51);
    // }

    
    // set traces
    Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
    csma[0].EnablePcap ("ecmp_ser", devices[0].Get (1), true); //n1
    csma[0].EnablePcap ("ecmp_tor", devices[0].Get (0), true); //t1
    csma[4].EnablePcap ("ecmp_aggr", devices[4].Get (0), true); //a1
    ppp[0].EnablePcap ("ecmp_core0", p2pDevices[0].Get (0), true); //c1
    ppp[1].EnablePcap ("ecmp_core1", p2pDevices[1].Get (0), true); //c2
